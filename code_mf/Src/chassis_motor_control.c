...
void rc_to_gimbal_speed_compute()
{
    ...
    {
        gimbal_vy_ros_vx = (0.25f * (robot_max_power - TOP_GIVE_MIN_SPEED ) ) ;
    }
    ...
    else
    {
        ...
        {
            //导航模式
            gimbal_vy_ros_vx = nav_rx_packet.vx * CMD_VEL_2_CHASSIS_SPEED_KP ;
        }
        else
        {
            //非导航模式
            gimbal_vy_ros_vx = (float)(4 * rc_ch1 ) ;
        }
    ...
    {
        gimbal_vx_ros_vy = -(0.25f * (robot_max_power - TOP_GIVE_MIN_SPEED ) ) ;
    }
    ...
    else
    {
        ...
        {
            //导航模式
            gimbal_vx_ros_vy = nav_rx_packet.vy * (-CMD_VEL_2_CHASSIS_SPEED_KP) ;
        }
        else
        {
            //非导航模式
            gimbal_vx_ros_vy = (float)(4 * rc_ch0 ) ;
        }
...
{
    chassis_vx = gimbal_vx_ros_vy * (float)cos((double)yaw_radian_difference) - gimbal_vy_ros_vx * (float)sin((double)yaw_radian_difference);
    chassis_vy = gimbal_vx_ros_vy * (float)sin((double)yaw_radian_difference) + gimbal_vy_ros_vx * (float)cos((double)yaw_radian_difference);
